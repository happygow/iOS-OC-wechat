...
- (void)buidUI
{
    ...
    ninePhotoView *npv = [[ninePhotoView alloc] initWithFrame:CGRectZero];
    [self addSubview:npv];
...
- (void)bindViewModel:(dgListCellModel *)vm
{
    ...
    NSArray *picArray = [vm.headImage componentsSeparatedByString:@","];
    self.npv.picAry = picArray;
    ...
    if (!kStringIsEmpty(vm.limage) && !kStringIsEmpty(vm.lcontent)) {
        self.npv.frame = CGRectMake(MIN_LEFTSPACETOAVATOR, MAX_CONTENTY + 10, SCREEN_WIDTH - MIN_LEFTSPACETOAVATOR - 70, [self collectionPiucViewHeight:picArray.count]);
    }
    else if (!kStringIsEmpty(vm.limage) && kStringIsEmpty(vm.lcontent))
    {
        self.npv.frame = CGRectMake(MIN_LEFTSPACETOAVATOR, MAX_USERNAMEY + 10, SCREEN_WIDTH - MIN_LEFTSPACETOAVATOR - 70, [self collectionPiucViewHeight:picArray.count]);
    }
    else
    {
...
- (CGFloat)collectionPiucViewHeight:(NSInteger)picCount
{
